...
#' @param noopt   Hold parameters fixed
#' @export
multinomialPotential <- function(dim = c(1,5), mat = NULL, independent = FALSE, pseudocount = 0L, noopt = FALSE){
  if(length(dim) != 2)
  ...
  pot <- list(mat = mat,
              param = list(independent = independent, pc = pseudocount, noopt = noopt))
